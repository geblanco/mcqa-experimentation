...
def get_params(args=None, throw=True):
    params = None
    if args is not None and args.params_file is not None:
...
def merge_with_params_file(parser, args):
    params = {}
    file_params = get_params(args, throw=False)
...
def parse_flags():
    ...
    parser.add_argument(
        "--params_file", required=False, default=None, type=str,
        help="Overwrite parameters with a nother file (default ./params.yaml)"
    )
